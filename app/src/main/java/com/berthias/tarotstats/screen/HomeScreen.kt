...
@Composable
fun PickRateRoi(modifier: Modifier = Modifier, listeParties: List<PartieUI>) {
...
@Composable
fun WinRateByRoi(modifier: Modifier = Modifier, listeParties: List<PartieUI>) {
    ...
    Column(modifier = modifier) {
        ...
        for (couleur in CouleurEnum.entries) {
            ...
            Text(text = "${couleur.stringValue}: ${"%.0f".format(winRate)}%")
        }
    }
}